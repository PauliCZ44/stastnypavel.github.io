function main()
{
  ...
  var moveY =2;
  var P1size = 16  * difficulty/1.2;  // orig = 15;
  ...
  if (this.x < Enemy.x + Enemy.size*0.7 && this.x + this.size*0.7 > Enemy.x && this.y < Enemy.y + Enemy.size*0.7 && this.y + this.size*0.9 > Enemy.y) {
      ...
      Enemy.size = random(EsizeMin+(P1size/5), EsizeMax+(P1size/1.5)); //as                   you grow the enemys are geting bigger
  ...
  var Enemy = function (x,y, size,Espeed){            //defining enemy
      ...
      this.Espeed = Espeed;  // i try to assign unique speed to every enemy but it does not work properly
      this.LR = 1;
  };
  ...
  draw = function() {
          ...
          if (player1.y >= (400-player1.size)){
              moveY = moveY*(-0.2);  // makes it bounce on Y = 400
          }
          if (player1.y <= (0.1)){  // makes you stop at Y = 0
                moveY = 0.2;

          }
          // Here i should add snaking of player//
          if (player1.x > width+10) {
             player1.x = player1.x-(width+10)-player1.size;
          }
          ...
          for (var i = 0; i < enemy2.length; i++){
              ...
              if  (enemy2[i].x > 400 && enemy2[i].LR === 1){
                  ...
                  enemy2[i].x = random (-500,-50);
                  enemy2[i].y = random (20,380);
                  enemy2[i].size = random(EsizeMin+(P1size/5), EsizeMax+(P1size/1.5)); //as                   you grow the enemys are geting bigger
                  var LR = floor(random(0,2));
                  enemy2[i].LR = LR;
      ...
      if (player1.size>EsizeMax*2.5) {
          ...
          player1.x = -50;
          textFont(f2,20);
          text("Your final score was: " + finalScore,200,25);
          fill (21, 0, 250);
          textFont(f);
      ...
      //This is millis() when the program first starts up...
              if (nowMillis < startMillis+350 && player1.size>1){
              textAlign(CENTER, CENTER);
              fill(255, 251, 0);
              textFont(f3,20);
      ...
      if (player1.size<1) {
          textFont(f2,20);
          text("Your score was: " + Score,200,25);
          fill (252, 0, 0);
          textFont(f);
          textAlign(CENTER, CENTER);
          textSize(80);
